...
def get_question(entry):
    ...
    return tlc_slice2BM25Task.norm(entry['question'])


@field_getter.add("qa")
def get_qa(entry):
    return tlc_slice2BM25Task.norm(f"{entry['question']} {entry['target']}")


@field_getter.add("a")
def get_decomp(entry):
    return tlc_slice2BM25Task.norm(entry['target'])


class tlc_slice2BM25Task:
    name = 'tlc_slice2'
