...
class ScorerDatasetReader(torch.utils.data.Dataset):
    ...
    def __init__(self, example_file, model_name, task_name, setup_type, ds_size=None, **kwargs) -> None:
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, local_files_only=True,truncation=True)
    ...
    def text_to_instance(self, entry: Dict[str, Any], index=-1):
        ...
        if self.setup_type == "qa":
            ...
            tokenized_example = self.tokenizer.encode_plus(enc_text, truncation=True, add_special_tokens=False,max_length=2000,
                                                           return_tensors='pt')
            tokenized_labels = self.tokenizer.encode_plus(test_answer, truncation=True, add_special_tokens=False,max_length=2000,
                                                          return_tensors='pt')
            ...
            tokenized_example = self.tokenizer.encode_plus(enc_text, truncation=True, add_special_tokens=False,max_length=2000,
                                                           return_tensors='pt')
            tokenized_labels = self.tokenizer.encode_plus(test_question, truncation=True, add_special_tokens=False,max_length=2000,
                                                          return_tensors='pt')
            # tokenized_example = self.tokenizer.encode_plus(enc_text, truncation=True, add_special_tokens=False,
            #                                                return_tensors='pt')
