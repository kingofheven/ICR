...
from get_trees import get_trees
...
class BiEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.coefficient4 = torch.nn.Parameter(torch.Tensor([0.5]))
    @staticmethod
    ...
    def forward(
        ...
        for i in question_ids:
            question_text=tensorizer.tokenizer.decode(i)
            question_text=question_text.split("[SEP]")[0].strip("[CLS]").strip()
            question_texts.append(question_text)
            question_tree=get_trees(question_text)
            question_trees.append(question_tree)
            question_trees_ids.append(tensorizer.text_to_tensor(question_tree))

        question_trees_ids=torch.stack(question_trees_ids).to(question_ids.device)
        question_trees_segments = torch.zeros_like(question_trees_ids).to(question_segments.device)
        question_trees_mask = tensorizer.get_attn_mask(question_trees_ids).to(question_attn_mask.device)

        _q_trees_seq, q_trees_pooled_out, _q_trees_hidden = self.get_representation(
            q_encoder,
            question_trees_ids,
            question_trees_segments,
            question_trees_mask,
            self.fix_q_encoder,
            representation_token_pos=representation_token_pos,
        )

        q_all_pooled_out=q_pooled_out*self.coefficient1+q_trees_pooled_out*self.coefficient2
        
        question_trees_ids.to("cpu")
        question_trees_segments.to("cpu")
        question_trees_mask.to("cpu")
        ...
        for i in context_ids:
            ctx_text=tensorizer.tokenizer.decode(i)
            ctx_text=ctx_text.split("[SEP]")[0].strip("[CLS]").strip()
            ctx_texts.append(ctx_text)
            ctx_tree=get_trees(ctx_text)
            ctx_trees.append(ctx_tree)
            ctx_trees_ids.append(tensorizer.text_to_tensor(ctx_tree))

        ctx_trees_ids=torch.stack(ctx_trees_ids).to(context_ids.device)
        ctx_trees_segments = torch.zeros_like(ctx_trees_ids).to(ctx_segments.device)
        ctx_trees_mask = tensorizer.get_attn_mask(ctx_trees_ids).to(ctx_attn_mask.device)

        _ctx_trees_seq, ctx_trees_pooled_out, _ctx_trees_hidden = self.get_representation(
            ctx_encoder, ctx_trees_ids, ctx_trees_segments, ctx_trees_mask, self.fix_ctx_encoder
        )
        ctx_all_pooled_out = ctx_pooled_out * self.coefficient3 + ctx_trees_pooled_out * self.coefficient4
        
        ctx_trees_ids.to("cpu")
        ctx_trees_segments.to("cpu")
        ctx_trees_mask.to("cpu")
        
        return q_all_pooled_out, ctx_all_pooled_out
