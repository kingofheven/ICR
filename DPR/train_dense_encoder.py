...
from dpr.models.biencoder import BiEncoder, BiEncoderNllLoss, BiEncoderBatch,BiEncoderBatch_list_ranking, BiEncoder_list_ranking_loss
...
class BiEncoderTrainer(object):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        assert self.loss_type in ['epr','list_ranking']
        ...
        logger.info('cfg.encoder.encoder_model_type:{}'.format(cfg.encoder.encoder_model_type))
    ...
    def run_train(self):
        ...
        cfg = self.cfg
        logger.info('cfg.train.gradient_accumulation_steps:{}'.format(cfg.train.gradient_accumulation_steps))
        ...
        eval_step = math.ceil(train_iterator.max_iterations / cfg.train.eval_per_epoch)
    ...
    def _train_epoch(
        ...
        logger.info('epoch_batches:{}'.format(epoch_batches))
        data_iteration = 0
        ...
        for i, samples_batch in enumerate(
            ...
            if self.loss_type == 'epr':
                ...
                biencoder_batch = BiEncoder.create_biencoder_input2_ranking(
                    samples_batch,
                    self.tensorizer,
                    True,
                    special_token
            ...
            if self.loss_type == 'epr':
                ...
                loss, list_ranking_loss, in_batch_negative_loss = _do_biencoder_fwd_pass_list_ranking(
                    self.biencoder,
                    biencoder_batch,
                    self.tensorizer,
                    cfg,
                ...
                step_rank_loss+=list_ranking_loss
                step_ib_loss+=in_batch_negative_loss
            ...
            if updated_step % log_result_step == 0:
                ...
                if self.loss_type == 'epr':
                    logger.info(
                        ...
                        epoch,
                        updated_step,
                        int(epoch_batches / cfg.train.gradient_accumulation_steps),
                    ...
                    logger.info(
                        "Epoch: %d: Step: %d/%d, loss=%f, rank_loss=%f, ib_loss=%f, lr=%f, ds=%s",
                        epoch,
                        updated_step,
                        int(epoch_batches / cfg.train.gradient_accumulation_steps),
...
def _calc_loss_list_ranking(
    ...
    loss, list_ranking_loss, in_batch_negative_loss = loss_function.calc(
        global_q_vector,
        global_ctxs_vector,
        ds_cfg.rank_loss_factor
    )

    return loss, list_ranking_loss, in_batch_negative_loss

def _do_biencoder_fwd_pass_list_ranking(
    model: nn.Module,
    input: BiEncoderBatch,
    tensorizer: Tensorizer,
    cfg,
    ...
    input = BiEncoderBatch_list_ranking(**move_to_device(input._asdict(), cfg.device))
    ...
    loss, list_ranking_loss, in_batch_negative_loss  = _calc_loss_list_ranking(
        cfg,
        ds_cfg,
        loss_function,
        local_q_vector,
    ...
    if cfg.train.gradient_accumulation_steps > 1:
        loss = loss / cfg.train.gradient_accumulation_steps
        list_ranking_loss = list_ranking_loss / cfg.train.gradient_accumulation_steps
        in_batch_negative_loss = in_batch_negative_loss / cfg.train.gradient_accumulation_steps
    return loss, list_ranking_loss, in_batch_negative_loss
